...
| 4 | P1 | Workspace batch edge cases | Add broader replacement graphs and concurrent dependency changes outside the guided candidate-selection path. Guided ambiguity diagnostics now carry an opaque revision of the exact workspace file population shown to the operator; every selected retry must echo it, and any intervening replacement/upload/delete fails closed as `source_document_workspace_dependency_selection_stale` until the ambiguity is refreshed. Memory/file/SQLite API coverage mutates a two-candidate relationship into three candidates between review and mutation, and production Chromium proves the stale warning, refresh, revised request, and successful immutable snapshot. Sequential upload mutation/collision behavior, the Original's single-file dependency deletion guard, and request-order-independent deletion of selected multi-level dependency graphs with failed-root retention are also closed. |
