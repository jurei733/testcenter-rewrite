...
| --- | --- | --- | --- |
| 1 | P0 | Original package dependency corpus expansion | A pinned 14.3/15.1/17.4/17.6 corpus now gates 24 booklet imports, all 17 official `CY_Bklt_TC-*` controller cases, all four `CY_Bklt_BkltConfig_*` variants, safety-critical XSD facets, and both prebuilt and loose multi-file execution of the original `Booklet2.xml` + `Unit2.xml` + coding scheme + Verona 6 dependency set; remaining work is production packages, complete XSD coverage, and more real players. |
