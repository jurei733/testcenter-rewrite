...
| 4 | P1 | Workspace file administration | Package-backed files are now browsable with exact byte size, import/release dependency counts, and protected byte-exact downloads; typed unpackaged file views, complete dependency graphs, guarded deletion, and replacement workflows remain. |
...
| File browser/upload/download/delete | workspace admin files module | partial | P1 | source-package cards expose stored byte size and import/release dependency counts, while a workspace-scoped operator route serves text and base64 ZIP uploads byte-exactly with safe attachment names; protected API tests cover all local stores and the Angular smoke compares downloaded ZIP bytes, but typed unpackaged files, dependency-aware deletion, and replacement remain |
