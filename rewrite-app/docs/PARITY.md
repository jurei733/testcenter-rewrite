...
| Adaptive booklet states | adaptivity E2E | partial | P0 | original XML states select the first matching option (with final fallback), evaluate IQB-standard Verona variables through `Value`, `Code`, `Score`, `Status`, `Sum`, `Mean`, `Median`, and nested `Count`, enforce `Show` routes server-side, and restrict the Angular unit menu to the active path; the server persists the original-equivalent `BOOKLET_STATES` snapshot at launch and after response saves, restores or backfills it across memory/file/SQLite/Postgres stores, and uses it authoritatively for participant and monitor routing; original Testtakers `Booklet state="…"` presets and differently preset variants of one booklet are validated, assigned stable identities, and exposed consistently to starter, player, history, monitor, and CSV; coding-scheme-derived variables remain |
