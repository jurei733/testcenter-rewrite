...
| 2 | P0 | Testlet locks and adaptivity | Original nested `Testlet` paths, participant `CodeToEnter` gates, server-authoritative `TimeMax` execution, durable `LockAfterLeaving` rules, and dimension-wise `DenyNavigationOnIncomplete` inheritance now extend the versioned runtime model; warning thresholds, monitor lock/time overrides, and adaptive states remain. |
...
| 4 | P1 | Monitor unlock/lock and time-adjustment commands | Pause, resume, complete, and audited go-to now exist; explicit whole-run unlock/lock plus operator-selected time restoration remain. |
...
| Go-to block, unlock, lock | monitor routes and E2E | partial | P1 | audited go-to sets the run to running, targets an exact unit, bypasses completion navigation guards, clears target code/leave locks, and reopens closed timed blocks with their configured duration; multi-run dispatch, operator-selected replacement time, and explicit whole-run unlock/lock remain |
