...
| 3 | P0 | Participant execution-mode completion | The six original participant modes now share an exact capability matrix and govern import, sessions, saving, restrictions, open-run/study-monitor visibility, remote commands, participant-authored test/unit/task-page reviews with priority, multi-category metadata, immutable browser/original-unit provenance, and editable adaptive paths in Demo/Review/Trial. Reopening a non-saving Demo, Review, or Simulation run now reuses its run but returns to the first currently visible Unit, matching the original's unsaved Unit/Test-state behavior; adaptive choices and Review comments remain available. The official 26-account Controller roster now independently exercises its four declared modes against the exact assigned original booklets as a complete workspace package. |
