...
| 2 | P0 | Testlet locks and adaptivity | Original nested `Testlet` paths, participant `CodeToEnter` gates, server-authoritative `TimeMax` execution, durable `LockAfterLeaving` rules, and dimension-wise `DenyNavigationOnIncomplete` inheritance now extend the versioned runtime model; global monitor navigation unlock and selected timed-block restoration exist, while warning thresholds, monitor re-lock, and adaptive states remain. |
...
| 4 | P1 | Monitor re-lock and bulk commands | Pause, resume, complete, audited go-to, whole-run navigation unlock, and operator-selected time restoration now exist; explicit re-lock and multi-run dispatch remain. |
...
| Booklet/unit navigation | `test-controller` | partial | P0 | ordered units, current position, nested original `Testlet` hierarchy, and per-unit testlet paths persist; imported `BookletConfig` compiles menu/button/player-end rules and server-side forward/backward eligibility, while original unit/testlet leave locks now prevent and visibly mark re-entry; conditional visibility remains |
...
| Timed blocks and warnings | time-restrictions E2E | partial | P0 | server-authoritative timers start on actual entry, persist across reload/storage, pause/resume durably, count down in the Angular player, expire into the next eligible unit, close against re-entry, enforce `forbidden`, `confirm`, and `allowed` navigation/completion rules, and accept audited operator-selected rest-time restoration; original warning presentation and detailed monitor timer visibility remain |
...
| Go-to block, unlock, lock | monitor routes and E2E | partial | P1 | audited go-to sets the run to running, targets an exact unit, bypasses completion navigation guards, clears target code/leave locks, and reopens closed timed blocks with their configured duration; whole-run unlock preserves run status and durably bypasses code, leave-lock, and completeness guards without reopening elapsed timed blocks; selected timed units accept replacement rest time that runs immediately only when current and otherwise waits for re-entry; multi-run dispatch and explicit re-lock remain |
