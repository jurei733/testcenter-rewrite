...
| 2 | P0 | Testlet adaptivity | Original adaptive state definitions, condition aggregations, `Show` routing, nested `Testlet` paths, participant-assignment state presets and same-booklet preset variants, `CodeToEnter` gates, server-authoritative `TimeMax` execution with configured warnings, durable `LockAfterLeaving` rules, and dimension-wise `DenyNavigationOnIncomplete` inheritance now extend the versioned runtime model; persisted Verona `BOOKLET_STATES` snapshots and server-side coding-scheme derivation remain. |
...
| Adaptive booklet states | adaptivity E2E | partial | P0 | original XML states select the first matching option (with final fallback), evaluate IQB-standard Verona variables through `Value`, `Code`, `Score`, `Status`, `Sum`, `Mean`, `Median`, and nested `Count`, enforce `Show` routes server-side, and restrict the Angular unit menu to the active path; original Testtakers `Booklet state="…"` presets and differently preset variants of one booklet are validated, assigned stable identities, snapshotted into each run, kept authoritative over response-derived values, and exposed consistently to starter, player, history, monitor, and CSV; persisted Verona `BOOKLET_STATES` and coding-scheme-derived variables remain |
...
| Original Testtakers XML | `XMLFileTesttakers.class.php` | partial | P0 | original sample fixture gates participant-versus-operational login modes, groups, passwords, ordered multiple-booklet assignments, and distinct identities for differently preset variants of the same booklet; codes, access windows, profiles, custom texts, and operator-account migration remain |
