...
| CI and deployability | deployment scripts | done | P0 | static, unit, storage, browser, startup/shutdown, Docker and Compose gates. All six Angular feature surfaces compile into guarded lazy chunks. After the Angular 21 upgrade, the frontend now relies on the framework's default zoneless change detection instead of shipping the redundant Zone.js runtime; the current production build drops from 597.76 kB to 561.60 kB raw initial JavaScript/CSS (127.00 kB estimated transfer), and dedicated Chromium gates cover async application settings, routed navigation, Participant SSE reconnect, and live monitor commands. Production budgets now warn at 570 kB and fail at 590 kB, so a Zone-sized initial-runtime regression cannot silently return while direct-entry and cross-feature browser paths remain executable |
