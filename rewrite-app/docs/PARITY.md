...
| 2 | P0 | Testlet timing, locks, and adaptivity | Original nested `Testlet` paths and participant `CodeToEnter` gates now extend the versioned runtime model; timed execution, leave-once locks, monitor unlock/lock, incomplete-testlet overrides, and adaptive states remain. |
...
| Timed blocks and warnings | time-restrictions E2E | missing | P0 | original `TimeMax` duration/leave metadata is imported, but no running timer, warning, expiry, or leave-policy enforcement exists yet |
