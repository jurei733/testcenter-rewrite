...
two-booklet Aspect, Group-Monitoring, and Session-Management graphs are fixed to upstream commit
...
are now pinned as 18.0 documents against those same current dependencies. Every
current upstream `sampledata` path and every changed member of the audited
Adaptive, Aspect, BookletConfig, Group-Monitoring, and Session-Management graphs are therefore
