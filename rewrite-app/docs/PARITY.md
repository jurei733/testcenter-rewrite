...
| 2 | P0 | Testlet locks and adaptivity | Original nested `Testlet` paths, participant `CodeToEnter` gates, server-authoritative `TimeMax` execution, and durable `LockAfterLeaving` rules for unit/testlet scopes now extend the versioned runtime model; warning thresholds, monitor lock/time overrides, incomplete-testlet overrides, and adaptive states remain. |
