...
| 5 | P1 | System check hardening | Original `SysCheck.xml` definitions now import without creating an empty test release; the lazy Angular flow captures environment data, measures application latency/network hints, renders all original questionnaire field types, hosts a resolved Verona unit, and saves key-protected reports. Controlled upload/download throughput, operator-login report saving, custom-text rendering, report statistics/deletion, and a pinned full player/unit dependency package remain. |
| 6 | P1 | Test-log export | Response, review, monitor, roster, directory, activity, and original-style dynamic system-check CSV exports exist; original participant/player test-log coverage remains incomplete. |
...
| System-check reports | sys-check module/routes | partial | P1 | the pinned original definition imports into a typed configuration, public direct links run environment/network/questionnaire/player stages, save-key validation protects durable reports across every store, scoped operators can list/filter/export them, and the dynamic semicolon CSV retains original report labels; report statistics/deletion, authenticated sys-check login bypass, custom texts, and exact legacy filenames remain |
