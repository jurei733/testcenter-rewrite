...
| 2 | P0 | Testlet adaptivity | Original adaptive state definitions, condition aggregations, `Show` routing, nested `Testlet` paths, participant-assignment state presets, `CodeToEnter` gates, server-authoritative `TimeMax` execution with configured warnings, durable `LockAfterLeaving` rules, and dimension-wise `DenyNavigationOnIncomplete` inheritance now extend the versioned runtime model; duplicate preset variants of the same booklet, persisted `BOOKLET_STATES` snapshots, and server-side coding-scheme derivation remain. |
...
| Original Testtakers XML | `XMLFileTesttakers.class.php` | partial | P0 | original sample fixture gates participant-versus-operational login modes, groups, passwords, ordered multiple-booklet assignments, and per-booklet adaptive state presets; duplicate differently preset variants of the same booklet, codes, access windows, profiles, custom texts, and operator-account migration remain |
