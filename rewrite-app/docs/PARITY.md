...
| 3 | P0 | Participant execution-mode completion | The six original participant modes now share an exact capability matrix and govern import, sessions, saving, restrictions, open-run/study-monitor visibility, remote commands, participant-authored test/unit reviews, and editable adaptive paths in Demo/Review/Trial. Original review priorities, multi-category selection, and task-page metadata remain. |
...
| Review create/edit/delete/export | review routes and review E2E | done | P1 | participant-authored test/unit comments are `canReview`-gated and browser-tested; operator CRUD, filters, readiness, audit activity, and CSV share the same durable records. Original priority, multi-category, and task-page metadata remain as fidelity refinements |
