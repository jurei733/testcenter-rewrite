...
| Text source encodings | XML file readers | partial | P0 | standalone uploads, loose dependency analysis, automatic assembly, file-type detection, and XML/manifest entries inside stored or deflated ZIPs share BOM-, XML-declaration-, and Data-URL-charset-aware decoding. Base64 and byte-preserving percent-encoded Data URLs both retain legacy source bytes. UTF-8, UTF-16LE, UTF-16BE, ISO-8859-1, and Windows-1252 aliases are supported without corrupting labels; original-corpus API gates cover standalone legacy bytes through both transport variants, a declaration-free charset transport, and a UTF-16 ZIP manifest. Remaining gaps are rarer XML encodings outside this compatibility set and encoding-declaration mismatch diagnostics |
