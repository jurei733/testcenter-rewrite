...
support, so the unmodified Player can embed the runtime and reload it in the
sandbox. This is a compatibility characterization, not a production-release
claim: the upstream repository is explicitly a feasibility study, and the
legacy runtime rejects the Player iframe's opaque `null`-origin controller
messages. The browser gate therefore proves nested HTML/CSS/JS/config delivery
and reload, while interactive response and state parity remain open.
