...
Latest P0 Booklet root-sequence closure: the importer now enforces the Original
XSD's direct `Metadata` → `CustomTexts` → `BookletConfig` → `States` → `Units`
order in addition to the existing generation-specific membership and singleton
checks. Out-of-order Booklet roots fail with the stable
`testcenter_xml_booklet_sequence_invalid` diagnostic before normalization can
silently accept them. Both direct XML intake and an XML dependency nested in an
IMS ZIP are pinned by the original compatibility corpus.
