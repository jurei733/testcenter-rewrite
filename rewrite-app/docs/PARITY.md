...
| 2 | P0 | Testlet locks and adaptivity | Original nested `Testlet` paths, participant `CodeToEnter` gates, server-authoritative `TimeMax` execution, durable `LockAfterLeaving` rules, and dimension-wise `DenyNavigationOnIncomplete` inheritance now extend the versioned runtime model; global monitor navigation unlock exists, while warning thresholds, monitor re-lock/time overrides, and adaptive states remain. |
...
| 4 | P1 | Monitor lock and time-adjustment commands | Pause, resume, complete, audited go-to, and whole-run navigation unlock now exist; explicit re-lock plus operator-selected time restoration remain. |
