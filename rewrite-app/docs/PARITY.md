...
| 3 | P0 | Participant execution-mode completion | The six original participant modes now share an exact capability matrix and govern import, sessions, saving, restrictions, open-run/study-monitor visibility, remote commands, participant-authored test/unit reviews with priority and multi-category metadata, and editable adaptive paths in Demo/Review/Trial. Original task-page review metadata remains. |
...
| Execution modes | `definitions/test-mode.json` | partial | P0 | all six participant modes use a versioned copy of the original capability matrix; Testtakers XML plus JSON/CSV aliases persist the mode through roster, session, run, SQLite/Postgres migrations, runtime state, monitoring, and CSV exports. `alwaysNewSession`, response/Player-log persistence, code/timer/navigation enforcement, menu/time visibility, open-run/study-monitor visibility, remote-command eligibility, editable state options, and participant-authored test/unit review CRUD with original priorities and multi-category selection are server-authoritative and API/browser-gated. Task-page review metadata remains. |
...
| Review create/edit/delete/export | review routes and review E2E | done | P1 | participant-authored test/unit comments are `canReview`-gated and browser-tested; original priorities `0–3` and simultaneous `tech`/`content`/`design` categories persist across memory/file/SQLite/Postgres storage, participant editing, operator cards, category filters, audit activity, and CSV. Task-page metadata remains as the fidelity refinement |
