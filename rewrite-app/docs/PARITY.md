...
| Player API compatibility validation | workspace file admin and unithost | partial | P0 | runtime ready handshake gates Verona major versions 2–6; ZIP import now rejects malformed/non-player JSON-LD metadata, unsupported API majors, and reference/id mismatches with stable diagnostics; complete Verona metadata-schema validation and metadata-absent legacy policy remain |
