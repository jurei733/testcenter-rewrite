...
| 2 | P0 | Testlet locks and adaptivity | Original nested `Testlet` paths, participant `CodeToEnter` gates, server-authoritative `TimeMax` execution, durable `LockAfterLeaving` rules, and dimension-wise `DenyNavigationOnIncomplete` inheritance now extend the versioned runtime model; global monitor navigation unlock/re-lock and selected timed-block restoration exist, while warning thresholds and adaptive states remain. |
