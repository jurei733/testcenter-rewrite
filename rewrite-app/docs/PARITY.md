...
navigation and reload. A separate production
...
| Capability | Original evidence | Rewrite status | Priority | Rewrite evidence / gap |
| --- | --- | --- | --- | --- |
| Independent Player-family gates | official ABI, DAN, STARS, and Speedtest repositories plus Verona Player Testbed | partial | P0 | provenance-pinned ABI 3.3.0, DAN 3.0.0, STARS 0.6.19, and Speedtest 1.2.0 releases import with official executable definitions and negotiate APIs 2.1, 4, and 6. ABI persists text/radio answers in string-valued `allResponses`; DAN persists positioned multiline-text/multiple-choice answers in string-valued `all`; STARS persists a choice in string-valued `responses` using `iqb-standard@1.1`; Speedtest persists its choice and elapsed time in string-valued `main` using `iqb-standard@1.0`. Production Chromium/SQLite reloads all four without Player-specific state rewriting, proving data-part value-kind preservation across distinct implementations and metadata generations. Speedtest additionally gates its mixed metadata-2.0/spec-5.0 declaration and runtime API-4 handshake, plain-text Unit definitions, automatic forward request, and final-Unit navigation denial. Separately, the complete current Original STARS 0.6.40 system-test graph executes its 28-alias Booklet and restores `iqb-standard@2.0` responses after Player-originated navigation. Further production families remain |
