...
- ZIP imports now inspect bundled Verona `application/ld+json` metadata before staging: malformed/non-player documents, unsupported API majors outside 2–6, and player-reference/version/id mismatches fail with stable diagnostics. Metadata-free legacy players remain importable with a stable warning; numeric versions inferred from their unit references are preflighted, and the runtime ready handshake remains authoritative. The original Verona 6 fixture, minimal metadata form, and metadata-free browser player are executable compatibility gates
