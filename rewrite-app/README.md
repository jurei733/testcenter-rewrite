...
- adaptive Testcenter booklets now import XML `States`, nested variable/aggregation conditions, and testlet `Show` restrictions; IQB-standard Verona response variables select the first matching state option with the original fallback behavior, and the server removes inactive units from participant menus, sequential navigation, code-gate scans, timed-block expiry routing, and completion while rejecting direct hidden-unit jumps
