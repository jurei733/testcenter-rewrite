...
- result-group administration reproduces the Original Testcenter's operator summary with started booklets, per-run minimum/maximum/average answered-unit counts, latest test activity, and response/review/log totals; one group can seed the detailed inspection scopes, while a separate multi-selection supports select-all, response/review/original-style log CSV exports, and workspace-key-confirmed aggregate deletion with an audit record. Response, review, and log reads accept repeated `groupKey` parameters without breaking the existing single-group filters; CSV routes allow a bounded 50,000-row export window while interactive lists remain capped at 500 rows
...
- a pinned original Testcenter 14.3/15.1/17.4/17.6 compatibility corpus now imports 27 booklets, including the equal-species `Booklet.xml`/`Booklet3.xml` pair, all 17 `CY_Bklt_TC-*` Test-Controller cases, all four `CY_Bklt_BkltConfig_*` variants, and both official Session-Management booklets with all five referenced Units and all 12 original logins; it verifies booklet identity, original top-level-testlet species, repeated-unit aliases, unit order, code gates, every `TimeMax` leave mode, global and nested completion rules, unit/testlet leave locks, password-free and password-protected entry, ordered multi-booklet assignments, second codes, access windows, and hot-return versus hot-restart re-entry with response restoration. The importer accepts the original `id-6.0` Verona-player reference as a version-checked legacy alias while retaining identity and version mismatch failures. Its real `Booklet2.xml` + `Unit2.xml` + coding scheme + Verona 6 dependency set gates both prebuilt-ZIP and loose multi-upload intake, immutable dependency retention, and raw-response adaptive routing end-to-end. Its complete original three-unit Aspect booklet, IQB Aspect 2.12.3 player, and all Unit/Voud pairs add a production-sized import and browser handshake/save/navigation/restore gate, including a 16.17 MB media definition; operational monitor/system-check logins are excluded from participant import
- the pinned original `SysCheck.xml` now imports as a first-class system-check definition without producing an empty content release. Separately uploaded SysCheck, Unit, IQB coding-scheme, and Verona files resolve transitively into one audited immutable package; its Unit definition and Player stay attached to the check, and a packaged missing Unit is rejected with a stable diagnostic. `/system-check` is a lazy direct-entry Angular flow for browser/device capture, configured repeated upload/download throughput against original-compatible `/speed-test/random-package` endpoints, application latency and connection hints, original threshold ratings, per-check `syscheck_*` custom texts, all questionnaire controls, a resolved Verona item, JSON download, and save-key-protected report persistence. Scoped operators can list/filter reports, export original-style semicolon CSV columns, inspect per-check operating-system/browser/overall-rating distributions and individual report values, and delete selected report sets only after typing the workspace key; deletion is store-scoped and audited, with API coverage across memory, file, and SQLite stores plus an Angular browser gate that starts the original Verona 6 fixture and retains its response in the report
...
- the production Angular shell now ships an installable same-origin web manifest and a scope-limited Service Worker. After one successful online visit, `/app/participant` can be opened or reloaded with the network disabled from versioned cached HTML, hashed bundles, lazy Participant chunks, styles, manifest, and icon; an accessible banner makes clear that sign-in, content loading, and saving still require connectivity. API routes, Participant current-state, Player definitions, resources, and responses are never cached. The server keeps HTML and the Service Worker revalidating while serving content-hashed JavaScript/CSS as immutable, and a dedicated Chromium smoke proves the navigation response came from the Service Worker during a real offline reload
