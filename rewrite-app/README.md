...
- original `TimeMax` restrictions now start a server-authoritative timer on actual block entry, survive reloads and durable-store restarts, pause and resume with the run, visibly count down in the Angular player, expire into the next eligible unit, permanently close elapsed or deliberately left blocks, and enforce the original `forbidden`, `confirm`, and `allowed` leave policies for navigation and participant completion; timer lifecycle activity is auditable and monitor pause/resume/complete commands share the same state model
- original `LockAfterLeaving` restrictions now enforce both `unit` and `testlet` scopes, honor optional participant confirmation, persist locked targets across reloads and durable-store restarts, prevent direct re-entry, skip locked units during sequential navigation, and expose the active rule plus locked units in the Angular player; original monitor-driven lock overrides remain a separate parity step
...
- monitor controls let authorized operators pause, resume, complete, or force an open run to a selected unit from the operator shell; supervised go-to bypasses participant navigation guards, clears target code/leave locks, reopens closed timed blocks with their configured duration, persists a `monitor_run_command_issued` workspace activity event, surfaces filterable command-history acknowledgements, and keeps the open-run/read-model state in sync
