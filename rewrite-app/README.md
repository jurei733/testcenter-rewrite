...
- review comments let operators attach, edit, list, delete, and export reviewer notes for concrete test runs or units, with review reads filterable by login, group, booklet, session, run, unit, reviewer, category, and limit; the runtime shell also derives a review-readiness checklist that joins answered units, missing responses, unit-level reviews, and whole-run reviews for the selected participant run, and its cards can seed matching response/review filters for follow-up inspection
- group result deletion removes collected test runs for one group, reports deleted runs/responses, records a workspace activity event, and the frontend requires operators to type the target group key before issuing the destructive action
...
- participant current-state can now deliver imported Verona player HTML and exact inline or referenced unit definitions; the Angular participant route runs supported Verona 2–6 players in a sandboxed frame, exchanges start/state/navigation/runtime-error messages, persists a versioned unit/player-state envelope through coalescing autosave with visible retry, and restores it after session reload; the browser smoke gate executes the pinned original Verona 6 sample, enters its real `var1`/`var2` responses, verifies the resulting adaptive route, and confirms response restoration after reload
- original `.itcr.zip` resource packages nested in import bundles are extracted with per-entry and aggregate size limits into immutable releases, served only through participant-session resource URLs, and exposed to Verona players through `playerConfig.directDownloadUrl`; the browser smoke gate imports the pinned original package and verifies that an originless sandboxed player can fetch its exact payload through CORS
...
- original `TimeMax` restrictions now start a server-authoritative timer on actual block entry, survive reloads and durable-store restarts, pause and resume with the run, visibly count down live in the Angular player when `unit_show_time_left` is enabled, show five-second alerts at compiled `unit_time_left_warnings` thresholds, expire into the next eligible unit, permanently close elapsed or deliberately left blocks, and enforce the original `forbidden`, `confirm`, and `allowed` leave policies for navigation and participant completion; timer lifecycle activity is auditable, monitor pause/resume/complete commands share the same state model, and operators can set a validated replacement rest time for a selected timed unit without moving or resuming its run
...
- a pinned original Testcenter 17.4/17.6 compatibility corpus now verifies booklet identity, repeated-unit aliases, unit order, participant login modes, ordered multi-booklet roster assignments, and the original `CY_Bklt_BkltConfig_2.xml` system-test matrix for navigation, timing, completion, restore, title, and fullscreen policies; its real `Booklet2.xml` + `Unit2.xml` + coding scheme + Verona 6 dependency set is assembled as a multi-file ZIP and gates immutable dependency retention plus raw-response adaptive routing end-to-end in both API and browser execution; operational monitor/system-check logins are excluded from participant import
- the pinned original `SysCheck.xml` now imports as a first-class system-check definition without producing an empty content release; `/system-check` is a lazy direct-entry Angular flow for browser/device capture, application-latency and connection-hint measurement, all original questionnaire controls, optional reuse of a resolved Verona unit/player, JSON download, and save-key-protected report persistence. Scoped operators can list/filter reports and export original-style semicolon CSV columns, with coverage across memory, file, and SQLite stores plus desktop/mobile browser execution
- the Angular participant route is emitted as a lazy production chunk, keeping the initial application bundle below its enforced budget while direct `/participant` links still load the complete player flow; browser smoke executes the booklet-requested fullscreen enter/exit path and verifies the configured header
- ZIP imports now inspect bundled Verona `application/ld+json` metadata before staging: malformed/non-player documents, unsupported API majors outside 2–6, and player-reference/version/id mismatches fail with stable diagnostics, while the original Verona 6 fixture and the existing minimal metadata form remain executable compatibility gates
- XSD-declaring original Booklet, Unit, and Testtakers XML, including XML entries nested in ZIP dependency bundles, is parsed as XML before normalization and checked for required schema structure, matching schema type, required identities, supported login modes, and duplicate group/login/testlet/unit runtime keys; failed imports expose stable diagnostic codes instead of staging permissively parsed content
