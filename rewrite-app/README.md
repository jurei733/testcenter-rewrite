...
The `smoke:ui:participant-review` variant verifies the participant starter's compact-height scroll helper plus its review download before and after comment creation, including the original empty-`204` feedback, stable `testcenter-reviews.csv` filename, Original-compatible CSV shape, and participant-owned content.
