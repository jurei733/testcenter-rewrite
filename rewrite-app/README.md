...
- participant review routes are bound to an opaque participant run, reject execution modes without `canReview`, and expose only that run's own test- and unit-level comments; Original Testcenter priorities `0–3` and simultaneous `tech`/`content`/`design` categories survive create, edit, storage, filtering, and reload
...
- review CSV export returns persisted comments with participant, run, booklet, unit, reviewer, priority, category list, compatibility category text, timestamp, and saved-roster display/assignment context, using the same filters as review reads
