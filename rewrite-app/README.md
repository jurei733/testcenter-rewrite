...
- Original Unit attachment variables with `format="capture-image"` survive packaged XML import and become requested image slots for each started run. The protected Attachment Manager lists missing/captured slots with participant and content context, constrains group monitors to assigned groups, accepts magic-byte-checked PNG/JPEG files up to 10 MiB, previews and deletes durable images, records upload/delete activity, and exposes a stable copyable handoff code. The remaining original-parity work is camera/QR scanning, printable QR/PDF sheets, and multipart capture-client compatibility
