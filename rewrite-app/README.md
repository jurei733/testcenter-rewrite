...
- ZIP imports now inspect bundled Verona `application/ld+json` metadata before staging: malformed/non-player documents, unsupported API majors outside 2–6, and player-reference/id mismatches fail with stable diagnostics, while the original Verona 6 fixture and the existing minimal metadata form remain executable compatibility gates
