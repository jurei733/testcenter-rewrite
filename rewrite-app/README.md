...
- original `TimeMax` restrictions now start a server-authoritative timer on actual block entry, survive reloads and durable-store restarts, pause and resume with the run, visibly count down in the Angular player, expire into the next eligible unit, permanently close elapsed or deliberately left blocks, and enforce the original `forbidden`, `confirm`, and `allowed` leave policies for navigation and participant completion; timer lifecycle activity is auditable, monitor pause/resume/complete commands share the same state model, and operators can set a validated replacement rest time for a selected timed unit without moving or resuming its run
