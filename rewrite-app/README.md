...
- original XML/JSON `BookletConfig` values now compile into a versioned runtime policy exposed by participant current-state; presentation/response completeness is enforced server-side for forward, backward, and completion requests, while the Angular/Verona player applies unit-menu/control, player-end, paging, logging, and page-restore settings with visible denial guidance
