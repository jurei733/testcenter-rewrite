...
- participant roster import now persists operator-managed login/group/booklet/display-name rows from CSV/TSV/semicolon text with canonical or common alias headers, Testtaker/Participant-style XML, original Testcenter `Testtakers`/`Login` XML, JSON roster text, or native JSON request objects/arrays, including nested Group/Booklet parent contexts, uses the same roster formats for direct participant-entry-link generation, upserts repeated logins within a workspace, records roster import activity, returns validation warnings when assigned booklets cannot be checked against the active release or are missing from it, feeds the frontend participant launchpad with a local roster parse preview plus generated-link start status, and exports saved roster rows as CSV
...
- workspace bootstrap
- source-package import, release activation, direct unit `Definition` prompt extraction, and roster-compatibility warning drill-downs for staged releases
- participant sign-in, session resume, direct `Definition` prompt display, and participant completion
- CSV/TSV/semicolon roster import with canonical or alias headers, XML/original Testcenter `Login`/JSON participant roster import, participant-session export, and open-run export through the runtime shell
...
- [ci.yml](/Users/julian/code/testcenter-rewrite/rewrite-app/.github/workflows/ci.yml) now verifies:
  ...
  - SQLite startup, shutdown, grouped quick/review/monitor/ops browser suites, full browser, and local-demo smokes as isolated matrix jobs
