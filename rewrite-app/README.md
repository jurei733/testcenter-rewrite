...
- a pinned original Testcenter 17.4/17.6 compatibility corpus now verifies booklet identity, repeated-unit aliases, unit order, selected runtime policies, participant login modes, and ordered multi-booklet roster assignments; its real `Booklet2.xml` + `Unit2.xml` + coding scheme + Verona 6 dependency set is assembled as a multi-file ZIP and gates immutable dependency retention plus raw-response adaptive routing end-to-end in both API and browser execution; operational monitor/system-check logins are excluded from participant import
...
- monitor controls let authorized operators pause, resume, complete, globally unlock or re-lock participant navigation, set a timed block's replacement rest time, or force an open run to a selected unit from the operator shell; global unlock durably bypasses code, leave-lock, and completeness guards without changing run status or reopening elapsed timed blocks, re-lock restores authored rules for subsequent actions without reconstructing already consumed one-time gates, replacement time is parked until entry when the block is not current, and supervised go-to targets an exact unit, clears its code/leave locks, and reopens its closed timed block with the configured duration; every command persists a `monitor_run_command_issued` workspace activity event, surfaces filterable history acknowledgements, and keeps the open-run/read-model state in sync
...
- participant sign-in, optional participant-password entry, session resume, direct `Definition` prompt display, and participant completion
