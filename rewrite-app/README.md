...
- a pinned original Testcenter 14.3/15.1/17.4/17.6 compatibility corpus now imports 24 booklets, including all 17 `CY_Bklt_TC-*` Test-Controller cases and all four `CY_Bklt_BkltConfig_*` variants; it verifies booklet identity, repeated-unit aliases, unit order, code gates, every `TimeMax` leave mode, global and nested completion rules, unit/testlet leave locks, participant login modes, and ordered multi-booklet roster assignments. Its real `Booklet2.xml` + `Unit2.xml` + coding scheme + Verona 6 dependency set is assembled as a multi-file ZIP and gates immutable dependency retention plus raw-response adaptive routing end-to-end in both API and browser execution; operational monitor/system-check logins are excluded from participant import
