...
- ten additional official `@iqb/responses` 3.6.0 rule families pin 30 byte-exact cases for text, regex, numeric boundary, boolean, null, empty-string, zero, empty-array, Player-injected variable coding, and intended-incomplete status propagation. One ten-booklet compatibility package gates `MATCH`, `MATCH_REGEX`, `NUMERIC_MATCH`, open and full numeric ranges, min/max comparisons, `IS_TRUE`, `IS_FALSE`, `IS_NULL`, `IS_EMPTY`, legacy preprocessing, residual zeroes, `INVALID`, `CODING_INCOMPLETE`, derived `SUM_SCORE`, pre-coded `CODING_COMPLETE`/`DISPLAYED`/`INVALID`/`CODING_ERROR` status with signed code/score values, and the exact derived outcome of `INTENDED_INCOMPLETE` combined with `CODING_INCOMPLETE`, `DERIVE_PENDING`, `UNSET`, `NOT_REACHED`, `DISPLAYED`, `PARTLY_DISPLAYED`, `DERIVE_ERROR`, `NO_CODING`, `INVALID`, `CODING_ERROR`, or a second `INTENDED_INCOMPLETE`. All 30 raw envelopes select an exclusive Participant route across memory, file, and SQLite
