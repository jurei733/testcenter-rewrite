...
versioned domain matrix across every storage adapter. Authored `Group/@label`
...
override matching starter and Player copy. The complete 43-key original
...
states resolve through it, including the original page-navigation prompt,
once-per-run console warning, controller-error message, and reload action. Active
Player failures now replace the complete Unit surface with
...
- original `TimeMax` restrictions now start a server-authoritative timer on actual block entry, survive reloads and durable-store restarts, pause and resume with the run, visibly count down live in the Angular player when `unit_show_time_left` is enabled, show five-second alerts at compiled `unit_time_left_warnings` thresholds, expire into the next eligible unit, permanently close elapsed or deliberately left blocks, and enforce the original `forbidden`, `confirm`, and `allowed` leave policies for navigation and participant completion; timer lifecycle activity is auditable, monitor pause/resume/complete commands share the same state model, operators can set a validated replacement rest time for a selected timed unit without moving or resuming its run, or atomically restore a closed target timer while jumping to that block. Open-run targets plus run-detail monitoring and CSV exports expose authored limits, timer labels, states, remaining time, leave policies, and lifecycle timestamps
...
- byte-preserving XML intake recognizes UTF-8, UTF-16, UTF-32, ISO-8859, Windows, and other legacy declarations across direct uploads, Data URLs, loose dependency assembly, and ZIP entries. XML-standard `ISO-10646-UCS-2` and `ISO-10646-UCS-4` aliases map onto the signature-aware UTF-16/32 decoders instead of being rejected as unknown; generic aliases accept either byte order only when the BOM or leading XML signature agrees. A production `iconv-lite` fallback extends Node's narrower WHATWG decoder set with allowlisted `ISO-8859-16`/Latin-10 and DOS `CP850` aliases without admitting transport codecs such as base64, hex, or UTF-7 as XML encodings. Original-corpus API gates preserve representative Romanian, German, and Euro labels across memory, file, and SQLite alongside a big-endian UCS-2 Booklet, a little-endian UCS-4 Booklet, and a UCS-4 IMS manifest inside ZIP
- ZIP imports now perform version-aware structural validation of bundled Verona `application/ld+json` metadata for the early `@id`/`@type`/`apiVersion` draft, the experimental `$schema`-only shape without `metadataVersion`, legacy 1.x/2.x, and strict 3.0/3.1 documents before staging: malformed/non-player documents, unsupported metadata or API versions, invalid core/nested fields, and player-reference/module-version/id mismatches fail with stable diagnostics. Unit references use `player-id@module-version`; `apiVersion` in the early draft and `specVersion` in later generations independently declare Verona API compatibility. Metadata-free legacy players remain importable with a stable warning because their reference cannot prove API compatibility, and the runtime ready handshake remains authoritative. The official `verona-player-simple` releases 1.0.1, 2.1.0, 4.0.0, and 5.2.0 negotiating APIs 2.1.0, 3.0.0, 4.0, and 5.2, the official ABI `3.3.0` scripted-survey Player and DAN `3.0.0` visual-assessment Player negotiating API `2.1.0`, both the independent STARS `0.6.19` pair and the complete current Original STARS `0.6.40` system-test package negotiating API `6.0`, the original Verona 6 fixture, the full original Aspect `2.12.3` module negotiating API `6.0`, all supported metadata generations, and a metadata-free browser player are executable compatibility gates
...
The named `smoke:ui:verona-player-families` slice stops after executing the official Simple Player generations, the independent ABI 3.3.0 scripted-survey and DAN 3.0.0 visual-assessment Players, the complete current Original STARS 0.6.40 system-test graph, and the historical metadata-free `IQBVisualUnitPlayerV2.99.2` Testbed graph. It gates type-faithful API-2 and API-6 response restoration after reload while leaving the separate PWA offline-shell check to its dedicated smoke slice.
...
The STARS corpus retains the byte-exact metadata-2.0/API-6 Player from a fixed official Testbed commit and a radio-button definition from the matching 0.6.19 source tag as independent-family coverage. Separately, the current Original Testcenter package pins the complete 0.6.40 Player/Unit/Voud/Vomd/28-alias Booklet/Testtakers graph. Its production SQLite/Chromium gate imports the unmodified assets, signs in through the original hot-return account, persists `iqb-standard@2.0` responses on aliases `1` and `2`, follows the Player's continue navigation, and restores the second selected option after reload.
