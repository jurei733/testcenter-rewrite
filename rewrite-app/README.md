...
states resolve through it, including the original controller-error message and
reload action. Active Player failures now replace the complete Unit surface with
...
The `smoke:ui:participant-review` variant verifies the participant starter's review download before and after comment creation, including the empty `204`, stable `testcenter-reviews.csv` filename, Original-compatible CSV shape, and participant-owned content.
