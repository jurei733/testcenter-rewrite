...
- monitor controls let authorized operators pause, resume, complete, globally unlock participant navigation, or force an open run to a selected unit from the operator shell; global unlock durably bypasses code, leave-lock, and completeness guards without changing run status or reopening elapsed timed blocks, while supervised go-to targets an exact unit, clears its code/leave locks, and reopens its closed timed block with the configured duration; every command persists a `monitor_run_command_issued` workspace activity event, surfaces filterable history acknowledgements, and keeps the open-run/read-model state in sync
