...
- loose source-package intake resolves unique original `Booklet` or `SysCheck` → `Unit` → Verona player, `DefinitionRef`, `VariablesRef`, coding-scheme, and player-resource references across already uploaded workspace files. Starting the root import captures that transitive closure in a new immutable dependency-snapshot ZIP, records exact member lineage, selects the snapshot in Angular, and runs the normal importer; ambiguous or incomplete matches stay on the explicit path. Manual assembly remains as the controlled fallback for 2–100 selected files, rejecting missing documents, unsafe relative paths, duplicate selection ids, case-insensitive duplicate archive paths, and per-file/aggregate size overflow while preserving an existing manifest or generating compatible IMS aliases from paths, original XML metadata, and Verona JSON-LD module identity/version metadata
