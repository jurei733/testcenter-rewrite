...
- byte-preserving XML intake recognizes UTF-8, UTF-16, UTF-32, ISO-8859, Windows, and other runtime-supported legacy declarations across direct uploads, Data URLs, loose dependency assembly, and ZIP entries. XML-standard `ISO-10646-UCS-2` and `ISO-10646-UCS-4` aliases now map onto the already signature-aware UTF-16/32 decoders instead of being rejected as unknown; generic aliases accept either byte order only when the BOM or leading XML signature agrees. Original-corpus API gates cover a big-endian UCS-2 Booklet, a little-endian UCS-4 Booklet, and a UCS-4 IMS manifest inside ZIP
