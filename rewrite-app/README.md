...
- Testtakers validation now follows the declared roster generation: monitor profile containers and references require schema 15.3, Booklet state presets plus extended monitor profile/filter fields require 15.4, and per-login `ViewSettings` require 17.6. Current 18.0 nested `ViewSettings` accept and validate the original `theme`, `codeInput` type/length, and `monitorBookletVisibility` structure without claiming unsupported participant-side theme or keypad application. The byte-exact original 15.2 E2E roster imports and updates idempotently, while cross-generation feature mixing fails with stable diagnostics across memory, file, and SQLite
