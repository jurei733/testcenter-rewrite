...
- nine additional official `@iqb/responses` 3.6.0 rule families pin 19 byte-exact cases for text, regex, numeric boundary, boolean, null, empty-string, zero, empty-array, and Player-injected variable coding. One nine-booklet compatibility package gates `MATCH`, `MATCH_REGEX`, `NUMERIC_MATCH`, open and full numeric ranges, min/max comparisons, `IS_TRUE`, `IS_FALSE`, `IS_NULL`, `IS_EMPTY`, legacy preprocessing, residual zeroes, `INVALID`, `CODING_INCOMPLETE`, derived `SUM_SCORE`, and pre-coded `CODING_COMPLETE`/`DISPLAYED`/`INVALID`/`CODING_ERROR` status with signed code/score values. All 19 raw envelopes select an exclusive Participant route across memory, file, and SQLite
