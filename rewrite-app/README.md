...
- participant roster import now persists operator-managed login/group/booklet/display-name rows from CSV/TSV/semicolon text with canonical or common alias headers, optional password/`pw` fields, Testtaker/Participant-style XML, original Testcenter `Testtakers`/`Login` XML, JSON roster text, or native JSON request objects/arrays, including nested Group/Booklet parent contexts and every ordered `<Login><Booklet>` assignment, hashes protected participant passwords internally, uses the same roster formats for direct participant-entry-link generation, upserts repeated logins within a workspace, records roster import activity, returns validation warnings for every assigned booklet that cannot be checked against the active release or is missing from it, feeds the frontend participant launchpad with a local roster parse preview plus generated-link start status, and exports saved roster rows as CSV with `passwordRequired` only
...
- a pinned original Testcenter 17.4/17.6 compatibility corpus now verifies booklet identity, repeated-unit aliases, unit order, selected runtime policies, participant login modes, and ordered multi-booklet roster assignments; operational monitor/system-check logins are excluded from participant import
...
- adaptive Testcenter booklets now import XML `States`, nested variable/aggregation conditions, and testlet `Show` restrictions; IQB-standard Verona response variables select the first matching state option with the original fallback behavior, and the server removes inactive units from participant menus, sequential navigation, code-gate scans, timed-block expiry routing, and completion while rejecting direct hidden-unit jumps; run detail and CSV exports expose the active state options to operators
