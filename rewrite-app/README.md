...
- participant roster import now persists operator-managed login/group/booklet/display-name rows from CSV/TSV/semicolon text with canonical or common alias headers, optional password/`pw` fields, Testtaker/Participant-style XML, original Testcenter `Testtakers`/`Login` XML, JSON roster text, or native JSON request objects/arrays, including nested Group/Booklet parent contexts, hashes protected participant passwords internally, uses the same roster formats for direct participant-entry-link generation, upserts repeated logins within a workspace, records roster import activity, returns validation warnings when assigned booklets cannot be checked against the active release or are missing from it, feeds the frontend participant launchpad with a local roster parse preview plus generated-link start status, and exports saved roster rows as CSV with `passwordRequired` only
