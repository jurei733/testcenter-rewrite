...
- eight additional official `@iqb/responses` 3.6.0 rule families pin 15 byte-exact cases for text, regex, numeric boundary, boolean, null, empty-string, zero, and empty-array coding. One eight-booklet compatibility package gates `MATCH`, `MATCH_REGEX`, `NUMERIC_MATCH`, open and full numeric ranges, min/max comparisons, `IS_TRUE`, `IS_FALSE`, `IS_NULL`, `IS_EMPTY`, legacy preprocessing, residual zeroes, `INVALID`, `CODING_INCOMPLETE`, derived `SUM_SCORE`, raw-response retention, and 15 exclusive Participant routes across memory, file, and SQLite
