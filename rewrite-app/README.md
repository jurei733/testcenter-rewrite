...
override matching starter and Player copy. The current 41-key Original
participant catalog from upstream `031b3313`, plus nine retained
legacy/runtime keys, is versioned as a 50-key compatibility contract including
the historical misspelling and `%s` placeholders. Current login-side copy,
...
states resolve through it. The current code gate renders its global prompt and
authored `CodeToEnter` message separately and no longer invents the removed
`booklet_codeToEnterWarning`; historical page-navigation, console-warning,
controller-error, and reload keys remain supported. Active
