...
export class ParticipantViewFacade {
  ...
  private applyEntryParameters(
    ...
    const tenantKeyProvided = parameters.tenantKey != null;
    const workspaceKeyProvided = parameters.workspaceKey != null;
    const normalized = {
      ...
      legacyShortLink: parameters.legacyShortLink === true
    };
    ...
    const scopeChanged =
      ...
      ((tenantKeyProvided && normalized.tenantKey !== previousTenantKey) ||
        (workspaceKeyProvided &&
          normalized.workspaceKey !== previousWorkspaceKey));

    if (tenantKeyProvided) {
      this.workspace.tenantKey = normalized.tenantKey;
    }

    if (workspaceKeyProvided) {
  ...
  private replaceLegacyShortLinkWithSessionEntry(
    ...
    globalThis.window.history.replaceState(
      ...
      "",
      buildParticipantSessionEntryUrl(
  ...
  private describeParticipantEntryIssue(
    ...
    switch (error.error) {
      case "participant_login_ambiguous":
        return {
          title: "Participant link ambiguous",
          detail: matchingWorkspaceCount
            ? `This login exists in ${matchingWorkspaceCount} workspaces.`
            : "This login exists in multiple workspaces.",
          action:
            "Open the complete participant entry link generated by the operator instead of the legacy short link.",
