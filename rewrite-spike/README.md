...
- a runnable long-lived `provider-operations` entrypoint that refreshes notification provider-profile operational state with active credential and target probes and records provider-operations audit events only when operational outcomes change
...
- a tenant-managed notification provider-profile registry plus partial workspace override records with live inheritance, explicit per-profile tombstones, per-profile provenance, enable/disable state, rollout-state metadata (`active`, `paused`, `canary`), credential-reference validation plus spike reachability checks, masked credential-reference metadata on read, computed provider health/readiness including `target_unreachable`, provider-operations-service-refreshed operational state (`lastCheckedAt`, actor provenance, rollout status, probe status, probe target, probe latency, last check error), and policy-history projection, so `profile:*` escalation targets can resolve into outbound delivery channel and target metadata without embedding provider config inside notification-policy overrides or leaking raw secret references back out through the API
...
4. deepen the dedicated provider-operations service with rollout-percentage controls, promotion workflows, and provider-specific probe-policy tuning beyond the current active-probe refresh model
