...
- a runnable long-lived `provider-operations` entrypoint that refreshes notification provider-profile operational state and records provider-operations audit events only when operational outcomes change
...
4. deepen the dedicated provider-operations service with active health probes, credential reachability refresh jobs, and rollout-percentage controls beyond the current state-refresh model
...
- `npm run start:provider-operations` owns notification provider-profile operational-state refresh and records audit events only when a provider profile's operational outcome changes; `--once` keeps a deterministic single-cycle mode for tests and debugging.
