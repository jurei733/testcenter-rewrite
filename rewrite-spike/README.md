...
- tenant-level notification-policy defaults with live workspace inheritance plus workspace override records that preserve per-field request and timestamp provenance for breach-notification channel selection, retry delay, and max delivery attempts, plus separate governance-notification-policy and recovery-governance-notification-policy families with the same inheritance and provenance model so incident alerts and resolved-incident recovery alerts can route independently of breach notifications and independently of each other
- tenant-level governance-case-policy defaults with live workspace inheritance plus workspace checklist-template, close-resolution-summary, structured close-resolution-field, queue-priority, and recommended-action override records that preserve per-rule request provenance for governance-case close guardrails, operator checklist items, severity-aware queue priority, and severity-aware operator guidance, now scoped by explicit governance-case family and optional case-severity overlays as well as case status
- a tenant-managed notification provider-profile registry plus partial workspace override records with live inheritance, explicit per-profile tombstones, per-profile provenance, enable/disable state, rollout-state metadata (`active`, `paused`, `canary`) with per-profile `rolloutPercentage` and fallback-profile routing, credential-reference validation plus spike reachability checks, per-profile `targetProbeMode`, masked credential-reference metadata on read, computed provider health/readiness including `target_unreachable`, provider-operations-service-refreshed operational state (`lastCheckedAt`, actor provenance, rollout status, probe status, probe target, probe latency, last check error), provider-profile incident state for auto-rollback suppression, a persisted workspace provider-incident queue with acknowledgement and resolution lifecycle, an operator-facing governance queue that combines unresolved incidents with rollout metrics and recommended actions, governance-case projections with queue/board views, assignment, SLA/escalation, workflow transitions, notes, checklist tracking, and close guardrails, a persisted governance-alert outbox with notification-service-driven delivery, operator acknowledgement flow, dedicated dead-letter inspection, and manual redrive with target correction, tenant/workspace notification-provider-promotion policy with live inheritance and partial overrides for evaluation windows, burn-in thresholds, automation flags, and suppression windows, workspace rollout metrics, promote workflow support with policy-driven guardrails plus optional force-promotion, and provider-operations auto-promote / auto-rollback decisions for workspace override profiles with suppression-aware recovery, so `profile:*` escalation targets can resolve into outbound delivery channel and target metadata without embedding provider config inside notification-policy overrides or leaking raw secret references back out through the API
...
- tenant and workspace policy-history projections that normalize policy changes from audit events into operator-facing activation, operational, launch-approval, notification, governance-case, notification-provider-profile, evidence-retention, and evidence-retention-class policy timelines
...
1. keep the spike frozen at the boundary defined in `../rewrite-production-baseline.md`
2. use this workspace as a semantic and contract reference while production code is extracted into the real implementation
3. allow only bug fixes, contract clarifications, and extraction-oriented refactors unless a true architecture blocker appears
4. shift new feature energy into the first production vertical slice instead of growing the spike further
...
- `GET /api/v1/platform/tenants/{tenantKey}/governance-notification-policy`
- `PATCH /api/v1/platform/tenants/{tenantKey}/governance-notification-policy`
- `GET /api/v1/platform/tenants/{tenantKey}/recovery-governance-notification-policy`
- `PATCH /api/v1/platform/tenants/{tenantKey}/recovery-governance-notification-policy`
- `GET /api/v1/platform/tenants/{tenantKey}/governance-case-policy`
- `PATCH /api/v1/platform/tenants/{tenantKey}/governance-case-policy`
...
- `GET /api/v1/tenants/{tenantKey}/workspaces/{workspaceKey}/governance-notification-policy`
- `PATCH /api/v1/tenants/{tenantKey}/workspaces/{workspaceKey}/governance-notification-policy`
- `GET /api/v1/tenants/{tenantKey}/workspaces/{workspaceKey}/recovery-governance-notification-policy`
- `PATCH /api/v1/tenants/{tenantKey}/workspaces/{workspaceKey}/recovery-governance-notification-policy`
- `GET /api/v1/tenants/{tenantKey}/workspaces/{workspaceKey}/governance-case-policy`
- `PATCH /api/v1/tenants/{tenantKey}/workspaces/{workspaceKey}/governance-case-policy`
...
- `GET /api/v1/tenants/{tenantKey}/workspaces/{workspaceKey}/notification-provider-profile-governance-queue`
- `GET /api/v1/tenants/{tenantKey}/workspaces/{workspaceKey}/notification-provider-profile-governance-alerts`
- `GET /api/v1/tenants/{tenantKey}/workspaces/{workspaceKey}/notification-provider-profile-governance-alert-metrics`
...
- `POST /api/v1/tenants/{tenantKey}/workspaces/{workspaceKey}/notification-provider-profile-governance-alerts/{alertId}:acknowledge`
- `POST /api/v1/tenants/{tenantKey}/workspaces/{workspaceKey}/notification-provider-profile-governance-alerts/{alertId}:redrive`
...
- `npm run start:provider-operations` owns notification provider-profile operational-state refresh with active credential and target probes, respects per-profile target-probe skip policy, persists probe-specific readiness details (`probeStatus`, `probeTarget`, `probeLatencyMs`), evaluates workspace-effective notification-provider-promotion policy for auto-promote and auto-rollback actions on workspace override profiles, opens persisted provider incidents plus governance alerts and suppression-aware profile incident state on auto-rollback, resolves queued incidents on later promotion, and records audit events only when a provider profile's operational outcome or rollout automation state changes; `--once` keeps a deterministic single-cycle mode for tests and debugging.
