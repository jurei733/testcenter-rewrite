...
import {
  ...
  type NotificationProviderProfileGovernanceCaseChecklistItemStatusDto,
  ...
  type AddNotificationProviderProfileGovernanceCaseNoteRequest,
  type AddNotificationProviderProfileGovernanceCaseNoteResponse,
  type UpsertNotificationProviderProfileGovernanceCaseChecklistItemRequest,
...
const workspaceNotificationProviderProfileGovernanceCaseAddNoteRoutePattern =
  /^\/api\/v1\/tenants\/([^/]+)\/workspaces\/([^/]+)\/notification-provider-profile-governance-cases\/([^/]+):add-note$/;
const workspaceNotificationProviderProfileGovernanceCaseUpsertChecklistItemRoutePattern =
  /^\/api\/v1\/tenants\/([^/]+)\/workspaces\/([^/]+)\/notification-provider-profile-governance-cases\/([^/]+):upsert-checklist-item$/;
...
const toNotificationProviderProfileGovernanceCaseBoardLane = (input: {
  ...
  displayLabel: string;
  items: WorkspaceNotificationProviderProfileGovernanceCaseQueueResponse["items"];
}): NotificationProviderProfileGovernanceCaseBoardLaneDto => ({
...
const handleWorkspaceNotificationProviderProfileGovernanceCaseBoardGet = async (
  store: PlatformStore,
  response: ServerResponse,
  url: URL,
  tenantKey: string,
  ...
  if (
    ...
    sendError(
      response,
      400,
      "invalid_notification_provider_profile_governance_case_board_case_status_filter",
      "caseStatus must be one of awaiting_incident_acknowledgement, suppressed_monitoring, awaiting_redrive, awaiting_alert_acknowledgement, recovered, or closed."
    );
    return;
  }
  ...
  if (slaStatus !== null && !isNotificationProviderProfileGovernanceCaseSlaStatus(slaStatus)) {
    sendError(
      response,
      400,
      "invalid_notification_provider_profile_governance_case_board_sla_status_filter",
      "slaStatus must be one of not_applicable, on_track, breached, or escalated."
    );
    return;
  }
  ...
  if (
    ...
    sendError(
      response,
      400,
      "invalid_notification_provider_profile_governance_case_board_assignment_status_filter",
      "assignmentStatus must be one of unassigned or assigned."
    );
    return;
  }
  ...
  const lanes: NotificationProviderProfileGovernanceCaseBoardLaneDto[] = [
    toNotificationProviderProfileGovernanceCaseBoardLane({
      laneKey: "needs_assignment",
      displayLabel: "Needs Assignment",
      items: queueItems.filter(item => item.caseItem.assignmentStatus === "unassigned")
    }),
    toNotificationProviderProfileGovernanceCaseBoardLane({
      laneKey: "breached_or_escalated",
      displayLabel: "Breached or Escalated",
      items: queueItems.filter(
        item =>
          item.caseItem.assignmentStatus === "assigned" &&
          (item.caseItem.slaStatus === "breached" ||
            item.caseItem.slaStatus === "escalated")
      )
    }),
    toNotificationProviderProfileGovernanceCaseBoardLane({
      laneKey: "awaiting_redrive",
      displayLabel: "Awaiting Redrive",
      items: queueItems.filter(
        item =>
          item.caseItem.assignmentStatus === "assigned" &&
          item.caseItem.caseStatus === "awaiting_redrive"
      )
    }),
    toNotificationProviderProfileGovernanceCaseBoardLane({
      laneKey: "awaiting_acknowledgement",
      displayLabel: "Awaiting Acknowledgement",
      items: queueItems.filter(
        item =>
          item.caseItem.assignmentStatus === "assigned" &&
          (item.caseItem.caseStatus === "awaiting_incident_acknowledgement" ||
            item.caseItem.caseStatus === "awaiting_alert_acknowledgement")
      )
    }),
    toNotificationProviderProfileGovernanceCaseBoardLane({
      laneKey: "monitoring_or_recovered",
      displayLabel: "Monitoring or Recovered",
      items: queueItems.filter(
        item =>
          item.caseItem.assignmentStatus === "assigned" &&
          (item.caseItem.caseStatus === "suppressed_monitoring" ||
            item.caseItem.caseStatus === "recovered")
      )
    })
  ];

  sendJson<WorkspaceNotificationProviderProfileGovernanceCaseBoardResponse>(
    response,
    200,
    {
      lanes,
...
const handleWorkspaceNotificationProviderProfileGovernanceCaseAddNote = async (
  store: PlatformStore,
  request: IncomingMessage,
  response: ServerResponse,
  tenantKey: string,
  ...
  const body = await readBody<AddNotificationProviderProfileGovernanceCaseNoteRequest>(request);
  const createdByActorId = getTrimmedString(body.createdByActorId);
  const noteBody = getTrimmedString(body.noteBody);

  if (!createdByActorId || !noteBody) {
    sendError(
      response,
      400,
      "invalid_notification_provider_profile_governance_case_note_payload",
      "createdByActorId and noteBody are required."
    );
    return;
  }

  const createdAt = new Date().toISOString();
  const noteId = `${incident.incidentId}:note:${createdAt}`;
  await recordAuditEvent(store, {
    requestId: requestContext.requestId,
    tenantId: workspace.tenantId,
    workspaceId: workspace.workspaceId,
    ...
    eventType: "workspace.notification_provider_profile_governance_case.note_added",
    payload: {
      workspaceKey,
      profileKey: incident.profileKey,
      incidentId: incident.incidentId,
...
const handleWorkspaceNotificationProviderProfileGovernanceCaseUpsertChecklistItem = async (
  store: PlatformStore,
  request: IncomingMessage,
  response: ServerResponse,
  tenantKey: string,
  ...
  const body =
    await readBody<UpsertNotificationProviderProfileGovernanceCaseChecklistItemRequest>(
      request
    );
  const itemKey = getTrimmedString(body.itemKey);
  const label = getTrimmedString(body.label);
  const status = body.status;
  const updatedByActorId = getTrimmedString(body.updatedByActorId);
  const note = getTrimmedString(body.note) ?? null;
  ...
  if (
    ...
    sendError(
      response,
      400,
      "invalid_notification_provider_profile_governance_case_checklist_item_payload",
      "itemKey, label, status, and updatedByActorId are required."
    );
    return;
  }
...
const handleRequest = async (
  ...
  const workspaceNotificationProviderProfileGovernanceCaseBoardRouteMatch = pathname.match(
    workspaceNotificationProviderProfileGovernanceCaseBoardRoutePattern
  );

  if (workspaceNotificationProviderProfileGovernanceCaseBoardRouteMatch) {
    const [, tenantKey, workspaceKey] =
      workspaceNotificationProviderProfileGovernanceCaseBoardRouteMatch;

    if (method === "GET") {
      await handleWorkspaceNotificationProviderProfileGovernanceCaseBoardGet(
        store,
        response,
        url,
        tenantKey,
  ...
  const workspaceNotificationProviderProfileGovernanceCaseAddNoteRouteMatch = pathname.match(
    workspaceNotificationProviderProfileGovernanceCaseAddNoteRoutePattern
  );

  if (workspaceNotificationProviderProfileGovernanceCaseAddNoteRouteMatch) {
    const [, tenantKey, workspaceKey, incidentId] =
      workspaceNotificationProviderProfileGovernanceCaseAddNoteRouteMatch;

    if (method === "POST") {
      await handleWorkspaceNotificationProviderProfileGovernanceCaseAddNote(
        store,
        request,
        response,
        tenantKey,
  ...
  const workspaceNotificationProviderProfileGovernanceCaseUpsertChecklistItemRouteMatch =
    pathname.match(
      workspaceNotificationProviderProfileGovernanceCaseUpsertChecklistItemRoutePattern
    );

  if (workspaceNotificationProviderProfileGovernanceCaseUpsertChecklistItemRouteMatch) {
    const [, tenantKey, workspaceKey, incidentId] =
      workspaceNotificationProviderProfileGovernanceCaseUpsertChecklistItemRouteMatch;

    if (method === "POST") {
      await handleWorkspaceNotificationProviderProfileGovernanceCaseUpsertChecklistItem(
        store,
        request,
        response,
        tenantKey,
